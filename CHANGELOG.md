...
## [3.5.0](https://github.com/TerryZ/v-page/compare/v3.4.0...v3.5.0) (2026-01-08)

### Features

- Completely refactored using `TypeScript`
- Global component registration now uses the default exported object, removing previously settable properties
  - Retains the `language` property; the language setting also applies to components not registered globally
  - Adds a `register` property to specify whether `PaginationBar` should be registered as a global component when the plugin is installed globally
